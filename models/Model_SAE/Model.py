...
class SAEModel(ModelBase):
    ...
    def onInitializeOptions(self, is_first_run, ask_override):
        ...
        if is_first_run or ask_override:
            self.options['stage'] = np.clip ( io.input_int("Stage (1 or 2 skip:%d) : " % (default_stage) , default_stage, help_message="Stage 1 is the main stage. Stage 2 is training to acquire higher details." ), 1, 2 )
        else:
    ...
    def onInitialize(self):
        ...
        self.DURNU = modelify ( SAEModel.DURNUFlow() ) ( Input(bgr_shape) )
        ...
        if self.is_training_mode:
            ...
            if stage == 2:
                pred_src_dst = self.DURNU(pred_src_dst*2-1) / 2 + 0.5
